...
def solve_restricted_hartree_fock(xyzfile, basis_name, charge, maxiter=150, enable_DIIS=True, verbose=True):
    ...
    if verbose:
        ...
        print(G_ao)
        print(np.sum(np.abs(G_ao)<1e-15))
...
def main():
    ...
    solve_restricted_hartree_fock(args.infile, args.basis, args.charge, enable_DIIS=(not args.no_diis), verbose=True)
