...
def load_basis(basis_name):
    ...
    basis_set = dict()
    reading = False
    basis_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)),os.pardir,'basis')
...
def integrate_pf_cartesian_pqrs(alpha_p, alpha_q, alpha_r, alpha_s, R_p, R_q, R_r, R_s, A_p, A_q, A_r, A_s):
    ...
    #global verbose
    #vv = False
    #if verbose is True and A_p[0] == 1 and A_p[2] == 1 and A_q[0] == 1 and A_q[2] == 1 \
    #                   and A_r[1] == 1 and A_r[2] == 1 and A_s[1] == 1 and A_s[2] == 1:
    #    vv = True
    #    verbose = False
    #    print "A_p", A_p, "A_q", A_q
    #    print "A_r", A_r, "A_s", A_s
    #    print "Rup", Rup
    #    print "Ruv", Ruv
    #    print 'w', w, 'u', u
    for xyz in range(3):
        ...
        #if vv:
        #    print "xyz = ", xyz
        #    print "start Aux[:,0,0,0,0] = ",Aux[:,0,0,0,0]
        ...
        for i in range(l_total):
            ...
            #if vv:
            #    print("adding I")
            #    print "Aux[%d,%d,%d,%d,%d] = %f*(%f-%f) = %f" % (N,i+1,0,0,0, i/(2*u), Aux[N,i-1,0,0,0], w/u*Aux[N+1,i-1,0,0,0], Aux[N,i+1,0,0,0])
            ...
            for k in range(-1, min(i+1, lr+ls)): # range from -1 to allow j to increase with lr=0 and ls=0
                ...
                if k >= 0:
                    ...
                    #if vv:
                    #    print("adding K")
                    #    print "Aux[%d,%d,%d,%d,%d] = %f" % (N,ki,0,k+1,0, Aux[N,ki,0,k+1,0])
                ...
                for j in range(-1, min(ki,lq)):
                    if j >= 0:
                        ...
                        #if vv:
                        #    print("adding J")
                        #    print "Aux[%d,%d,%d,%d,%d] = %f" % (N,ki-j-1,j+1,k+1,0, Aux[N,ki-j-1,j+1,k+1,0])
                    ...
                    for l in range(min(k+1,ls)):
                        ...
                        #if vv:
                        #    print("adding L")
                        #    print "Aux[%d,%d,%d,%d,%d] = %f" % (N,ki-j-1,j+1,k-l,l+1, Aux[N,ki-j-1,j+1,k-l,l+1])
        # finish current xyz
...
def test1():
    #basis_set = load_basis('sto-3g')
    #elems = ['H','H','O']
    #coords = [(0., -1.43233673, - 0.96104039),
    #          (0., 1.43233673, - 0.96104039),
    #          (0., 0., 0.24026010) ]
    basis_set = load_basis('only_d')
    elems = ['O']
    coords = [(0., 0., 0.) ]
    ...
    print G_ao.shape
    nbf = len(G_ao)
    #idx = np.array([0,1,2,3,6,4,5])
    #G_ao = G_ao[idx,:,:,:][:,idx,:,:][:,:,idx,:][:,:,:,idx]
    column = 0
    print("Column", column)
    for i,e in enumerate(G_ao.reshape(nbf**2,nbf**2)[column,:]):
        print "%5d  %13.7f" %(i+1, e)

    import pickle
    pickle.dump(G_ao, open("g_ao.data", 'wb'))
